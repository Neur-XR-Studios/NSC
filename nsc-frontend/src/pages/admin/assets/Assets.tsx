...
export default function Assets() {
  ...
  ): Promise<ApiEnvelope<{ total?: number; page?: number; limit?: number; data?: JourneyItem[] }>> => {
  ...
  const handleDelete = async (item: JourneyItem) => {
    ...
    try {
      ...
      const errorMessage =
        error && typeof error === "object" && "message" in error ? String(error.message) : "Failed to delete journey";
                  ...
                  <div className="p-3 flex flex-col gap-2">
                      ...
                      {it.languages?.length ? (
                        <div className="flex gap-1">
                          {it.languages.slice(0, 2).map((lng) => (
                            <span key={lng} className="text-[10px] px-1.5 py-0.5 rounded bg-cyan-500/20 text-cyan-500">
                              {lng}
                            </span>
                          ))}
                    ...
                    <div className="flex items-center justify-end gap-2 pt-1">
                      ...
                      <Button
                        ...
                        variant="outline"
                        className={
                          customCss.buttonOutline +
                          " h-8 w-8 hover:bg-red-500/10 hover:text-red-500 hover:border-red-500"
                        }
