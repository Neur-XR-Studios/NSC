...
class SessionService {
  ...
  async listSessions({ page = 1, limit = 20, status = 'on_going' }) {
    ...
    // Build vr/chair device maps from participants for all sessions (batch), but do not expose participants
    const vrBySession = new Map();
    const chairBySession = new Map();
    if (allSessionIds.length > 0) {
      ...
      for (const p of participants) {
        ...
        if (!vrBySession.has(sid)) vrBySession.set(sid, []);
        if (!chairBySession.has(sid)) chairBySession.set(sid, []);
        const pv = p.vr && (p.vr.toJSON ? p.vr.toJSON() : p.vr);
        const pc = p.chair && (p.chair.toJSON ? p.chair.toJSON() : p.chair);
        if (pv && !vrBySession.get(sid).find(v => v.id === pv.id)) vrBySession.get(sid).push(pv);
        if (pc && !chairBySession.get(sid).find(c => c.id === pc.id)) chairBySession.get(sid).push(pc);
      }
    }
    ...
    const enriched = rows.map((s) => {
      ...
      if (plain.vr) {
        if (Array.isArray(plain.vr)) {
          vrDevices.push(...plain.vr);
        } else {
          vrDevices.push(plain.vr);
        }
      }
      if (plain.chair) {
        if (Array.isArray(plain.chair)) {
          chairDevices.push(...plain.chair);
        } else {
          chairDevices.push(plain.chair);
        }
      }

      // If still empty, fill from participants mapping
      if (vrDevices.length === 0 && vrBySession.has(plain.id)) {
        vrDevices.push(...vrBySession.get(plain.id));
      }
      if (chairDevices.length === 0 && chairBySession.has(plain.id)) {
        chairDevices.push(...chairBySession.get(plain.id));
      }

      // participants are not included in list response
  ...
  commandSession({ sessionId, cmd, positionMs, durationMs }) {
    const topic = `session/${sessionId}/cmd`;
    ...
    switch (cmd) {
      case 'start':
        ...
        break;
      case 'pause':
        payload = { cmd: 'pause', atPositionMs: positionMs };
        break;
      case 'stop':
        payload = { cmd: 'stop' };
        break;
